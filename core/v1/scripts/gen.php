...
if ($dbh->connected) {
   ...
   while ($create-- > 0) { 
      $userid    = 1001;
      $thiskey   = $auth->generateKey($userid);
      $thistoken = $auth->generateToken($apikey);
      $limitrate = 100;
      $limitconc = 10;

      $sql1 = "insert into api_key (apikey,requestor_user_id,jsonrbac,limit_rate,limit_concurrent) values ('%s',%d,'%s',%d,%d)";
      $insert1 = sprintf($sql1,$thiskey,$userid,'["readonly"]',$limitrate,$limitconc);
      $sql2 = "insert into api_token (apikey,apitoken,jsonaccess,limit_rate,limit_concurrent,expires) values ('%s','%s','%s',%d,%d,now() + interval 1 year)";
      $insert2 = sprintf($sql2,$thiskey,$thistoken,'[".*:GET"]',$limitrate,$limitconc);

      $rc1 = $dbh->execute($insert1);
      $rc2 = $dbh->execute($insert2);
