...
if ($dbh->connected) {
   ...
   foreach ($results as $apikey => $info) {
      ...
      $sha1token = hash('sha1',$apitoken,false);
      $createsql = "CREATE TABLE token_%s LIKE api_token";
      $createsql2 = "INSERT token_%s SELECT * FROM api_token where apitoken = '%s';";
      $rc = $dbh->execute(sprintf($createsql,$sha1token));
      $rc2 = $dbh->execute(sprintf($createsql2,$sha1token,$apitoken));

/*
      if (!$rc) {
         list($errno,$error) = $dbh->error();
         print "[$errno] $error\n";
      }
