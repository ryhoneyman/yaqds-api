...
if ($dbh->connected) {
   ...
   $findsql = "select * from api_token where expires > now() + interval 1 month limit $findcount";
   ...
   foreach ($results as $apikey => $info) {
      ...
      $filedata = preg_replace('/{APITOKEN}/',$apitoken,$tmpl);
      $filedata = preg_replace('/{COUNT}/',$count,$filedata);
      print $filedata; 
   }
}
